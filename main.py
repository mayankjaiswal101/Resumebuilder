...
with col1:
    ...
    with st.container(border=True):
        ...
        full_name = st.text_input("Full Name", placeholder="")
        email = st.text_input("Email", placeholder="")
        phone = st.text_input("Phone", placeholder="")
        linkedin = st.text_input("LinkedIn", placeholder="")
        location = st.text_input("Location", placeholder="")
    with st.container(border=True):
        education = st.text_area("Education", placeholder="", height=80)
        skills = st.text_area("Skills", placeholder="", height=80)
        projects = st.text_area("Projects", placeholder="", height=80)
        experience = st.text_area("Experience", placeholder="", height=80)

with col2:
    st.markdown("### Live Preview - White Theme")
    ...
    <div class="resume-card">
        <h2 style="margin:0; text-align:center;">{full_name}</h2>
        <p style="color:#555; font-size:13px; text-align:center;">{email} | {phone} | {location}<br>{linkedin}</p>
        <div style="background:#F9FAFF; border:1px solid #EAEAEA; border-radius:8px; padding:12px; margin-top:15px;">
            ...
            <p style="font-size:14px; margin:0;">{education}</p>
        </div>
        <div style="background:#F9FAFF; border:1px solid #EAEAEA; border-radius:8px; padding:12px; margin-top:12px;">
            ...
            <p style="margin:0;">{skill_html}</p>
        </div>
        <div style="background:#F9FAFF; border:1px solid #EAEAEA; border-radius:8px; padding:12px; margin-top:12px;">
            <h4 style="color:#6C63FF; margin:0 0 5px 0;">PROJECTS</h4>
            <p style="font-size:14px; margin:0;">{projects}</p>
        </div>
        <div style="background:#F9FAFF; border:1px solid #EAEAEA; border-radius:8px; padding:12px; margin-top:12px;">
            <h4 style="color:#6C63FF; margin:0 0 5px 0;">EXPERIENCE</h4>
            <p style="font-size:14px; margin:0;">{experience}</p>
        </div>
    </div>
    """
    ...
    if st.button("Download WHITE PDF", use_container_width=True, type="primary"):
        if not full_name:
            ...
            href = f'<a href="data:application/octet-stream;base64,{b64}" download="Resume.pdf"><div style="width:100%; padding:14px; background:#6C63FF; color:white; text-align:center; border-radius:10px;">Download</div></a>'
